  ...
  if (state.kind === "pending") {
    return (
      <div className="flex items-center gap-2 text-sm text-kite-fg/80">
        <Loader2 className="w-4 h-4 animate-spin text-kite-primary" />
        <span>Submitted, waiting for confirmation</span>
        <a
          href={explorerTxUrl(state.hash, network)}
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1 font-mono text-xs text-kite-primary hover:text-kite-fg transition-colors"
        >
